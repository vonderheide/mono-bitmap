        ...
        Class.fromFile = function (filePath) {
            ...
            if (!hasError) {
                ...
                while (filePosition < header.dataOffset) {
                    let bytesRead = fs.readSync(file, fileBuffer, 0, fileBuffer.length, null);
            ...
            if (!hasError) {
                ...
                for (let y = header.height - 1; y >= 0; y--) {
                    let bytesRead = fs.readSync(file, fileBuffer, 0, numberOfBytesPerLine,
                        null);
                    if (bytesRead !== numberOfBytesPerLine) {
                        hasError = true;
                        errorMessage = `Unexpected end of file in "${filePath}"`;
                        break;
                    }
...
let Font =  // eslint-disable-line no-unused-vars
    /**
